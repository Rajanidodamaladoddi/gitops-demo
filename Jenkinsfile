pipeline {
    agent any
    environment {
        DOCKERHUB_USERNAME = "Rajaninandu"
    ...
    stages {
        ...
        stage('Push the changed deployment file to Git'){
            steps {
                script{
                    sh """
                    git config --global user.name "Rajanidodamaladoddi"
                    ...
                    //sh "git push http://$user:$pass@github.com/Rajanidodamaladoddi/gitops-demo.git master"
                    withCredentials([gitUsernamePassword(credentialsId: 'Gitlogin', gitToolName: 'Default')]) {
                    sh  "git push https://github.com/Rajanidodamaladoddi/gitops-demo.git HEAD:master"
                    }
                }
            }
        }
